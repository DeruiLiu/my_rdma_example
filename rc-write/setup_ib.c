...
struct ibcontext *ib_init_ctx(struct ibv_device *ib_dev, unsigned long long size, int rx_depth, int ib_port)
{
...

    {
        struct ibv_qp_attr attr = {
            ...
            .pkey_index = 0,
            .port_num = ib_port,
            .qp_access_flags = IBV_ACCESS_REMOTE_WRITE | IBV_ACCESS_LOCAL_WRITE | IBV_ACCESS_REMOTE_READ
        };
...
int info_post_recv(struct ibcontext *ctx,int info_size){
    struct ibv_sge list = {
        .addr = (uintptr_t)ctx->buf,
        .length = info_size,
        .lkey = ctx -> mr->lkey,
