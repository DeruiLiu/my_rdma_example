...
struct ibcontext *ib_init_ctx(struct ibv_device *ib_dev, unsigned long long size, int rx_depth, int ib_port)
{
    ...
    ctx = calloc(1, sizeof *ctx);//分配内存空间
    if (!ctx)
        return NULL;
...

    {
        struct ibv_qp_attr attr = {
            ...
            .pkey_index = 0,
            .port_num = ib_port,
            .qp_access_flags = IBV_ACCESS_REMOTE_WRITE | IBV_ACCESS_LOCAL_WRITE | IBV_ACCESS_REMOTE_READ//QP也有对端的权限操作
        };
...
int info_post_recv(struct ibcontext *ctx,int len){
        struct ibv_sge list = {
        .addr = (uintptr_t)ctx->buf,
        .length = len,
        .lkey = ctx -> mr->lkey,
...
int info_post_send(struct ibcontext *ctx,int wr_id,int len){
    struct ibv_sge list={
        .addr = (uintptr_t)ctx->buf,
        .length = len,
        .lkey = ctx->mr->lkey
