...
int main(int argc, char *argv[])
{
    ...
    char *send_buf_ptr = ctx->buf;//此处为本端的地址
    for(int i = 0;i < 2;i ++){
        ctx->send_sges[i].addr = (uintptr_t) send_buf_ptr;
        ctx->send_sges[i].length = size;
    ...
    for(i = 0; i < 1; i ++){
        ...
        ctx->send_wrs[i].sg_list = &ctx->send_sges[i];
        ctx->send_wrs[i].num_sge = 1;
        ctx->send_wrs[i].wr_id = IB_WRITE_WRID;
        ctx->send_wrs[i].opcode = IBV_WR_RDMA_WRITE;
        ctx->send_wrs[i].wr.rdma.remote_addr = raddr;
        ctx->send_wrs[i].wr.rdma.rkey = ctx->rkey;
        raddr += size; 
    }
        ctx->send_wrs[i].next = NULL;
        ctx->send_wrs[i].sg_list = &ctx->send_sges[i];
        ctx->send_wrs[i].num_sge = 1;
        ctx->send_wrs[i].wr_id = IB_WRITE_WRID;
        ctx->send_wrs[i].opcode = IBV_WR_RDMA_WRITE_WITH_IMM;//最后一块数据通知对端写数据完成
        ctx->send_wrs[i].wr.rdma.remote_addr = raddr;
        ctx->send_wrs[i].wr.rdma.rkey = ctx->rkey;
        raddr += size; 
    ...
    while (scnt < iters) {
        ...
        for (int i = 0; i < ne; i++) {
            ...
            if ((int)wc[i].wr_id == IB_WRITE_WRID) {
                ++scnt;
            } else {
