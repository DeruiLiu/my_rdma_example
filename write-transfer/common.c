...
void build_qp_attr(struct ibv_qp_init_attr *qp_attr)
{
    ...
    qp_attr->cap.max_send_wr = 10; //发送队列的最大深度为10
    qp_attr->cap.max_recv_wr = 10; //接收队列的最大深度为10
    ...
    while (1)
    {
        ...
        while(ibv_poll_cq(cq,1,&wc)){
            if (wc.status == IBV_WC_SUCCESS)
                s_on_completion_cb(&wc);//此处调用函数指针，即当收到一个cqe后去判断是否要继续发送数据
            else
            {
...
void build_context(struct ibv_context *verbs)
{
    ...
    s_ctx = (struct context *)malloc(sizeof(struct context)); //给指针分配内存
    ...
    TEST_NZ(ibv_req_notify_cq(s_ctx->cq, 0)); //为指定的cq提供事件通知机制
...
void event_loop(struct rdma_event_channel *ec, int exit_on_disconnect)
{
    ...
    while (rdma_get_cm_event(ec, &event) == 0)
    {
        ...
        if (event_copy.event == RDMA_CM_EVENT_ADDR_RESOLVED)//建立连接
        {
            build_connection(event_copy.id);
            if (s_on_pre_conn_cb)//函数指针,为了client与server相同的逻辑，函数指针调用不同的函数
        ...
        {
            TEST_NZ(rdma_connect(event_copy.id, &cm_params));//此时只是互换了QP信息，还没有互换地址信息
        }else if(event_copy.event ==  RDMA_CM_EVENT_CONNECT_REQUEST){//此时需要重新选择新的id
            build_connection(event_copy.id);
            ...
            if(s_on_connect_cb)
                s_on_connect_cb(event_copy.id);//对于client来说没有on_connect，对于server来说此时通过send发送地址信息给client

        }else if(event_copy.event == RDMA_CM_EVENT_DISCONNECTED){
            rdma_destroy_qp(event_copy.id);

            if(s_on_disconnect_cb)
                s_on_disconnect_cb(event_copy.id);//对于client来说没有dis_connect
