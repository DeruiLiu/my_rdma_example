...
//gcc -g -Wall -D_GNU_SOURCE -g -O2 -o multicast  multicast.c -libverbs -lrdmacm
#include <stdlib.h>
#include <string.h>
#include <stdio.h>
#include <errno.h>
...
static int post_recvs(struct cmatest_node *node)
{
    ...
    for (i = 0; i < message_count && !ret; i++) {//接收端首先将WQE塞满接收队列
...
static int post_sends(struct cmatest_node *node, int signal_flag)
{
    ...
    while(scnt < iters){
        ...
        int done,rec = 0;
        for (done = 0; done < message_count; done += rec) {
            rec = ibv_poll_cq(node->cq, 8, wc);
            if (rec < 0) {
...
static int addr_handler(struct cmatest_node *node)
{
    ...
    ret = rdma_join_multicast(node->cma_id, test.dst_addr, node);//加入一个组播组
    if (ret) {
...
{
    int ret, i;
    test.nodes = malloc(sizeof *test.nodes * connections);//分配了一块内存
    if (!test.nodes) {
...
static int my_post_recv(struct cmatest_node *node)
{
    struct ibv_recv_wr recv_wr, *recv_failure;
    struct ibv_sge sge;
    int i, ret = 0;
...
static int poll_cqs(void)
{
    ...
    for (i = 0; i < connections; i++) {
        ...
        while(rcnt <iters){
            ...
            //static int post_recvs(struct cmatest_node *node)
            ret = post_recvs(&test.nodes[i]);
            if(ret){
                printf("recv post recvs failed \n");
                return 1;
            }
            rcnt ++;
        }
...
static int connect_events(void)
{
    ...
    while (test.connects_left && !ret) {
        ret = rdma_get_cm_event(test.channel, &event);//0表示成功
        if (!ret) {
...
static int run(void)
{
    ...
    for (i = 0; i < connections; i++) {
        if (src_addr) {//将自己的ip地址与组播地址进行绑定
            ret = rdma_bind_addr(test.nodes[i].cma_id, test.src_addr);//将源地址与RDMA_CM_ID关联，如果绑定到一个特定的本地地址，RDMA_CM_ID也将绑定到一个本地rdma设备
            if (ret) {
        ...
        if (unmapped_addr)//M
            ...
            //将目标地址和可选源地址从IP地址解析为RDMA地址，如果成功，指定的RDMA_CM_ID将绑定到本地设备
        if (ret) {
...
int main(int argc, char **argv)
{
    ...
    while ((op = getopt(argc, argv, "m:M:s:b:c:C:S:p:n:")) != -1) {
        switch (op) {
            case 'm':
                dst_addr = optarg;
                break;
    ...
    test.dst_addr = (struct sockaddr *)&test.dst_in;//啥意思？
    test.connects_left = connections;//应该是一共有几个连接
    test.channel = rdma_create_event_channel();//打开一个用于报告通信事件的事件通道，异步事件通过事件通道报告给用户
    if (!test.channel) {
    ...
    destroy_nodes();
    float usec = (end1.tv_sec - start.tv_sec) * 1000000 + (end1.tv_usec - start.tv_usec);
    long long bytes = (long long) iters * message_count * message_size;

    printf("time1_gap is %f us\n",usec);
